...
For a Medium-ready English article, paste from **`docs/medium_article.md`**. For a shorter Spanish Word draft, regenerate with ``scripts/build_publication_docx.py`` → **`docs/publication.docx`**. The technical manuscript **`docs/paper.docx`** includes an addendum on benchmark integrity (single-thread decode parity).
...
└── docs/
    ...
    ├── medium_article.md          # English Medium paste draft (copy entire file)
    ├── publication.docx         # shorter Spanish Word draft; scripts/build_publication_docx.py
